...
def ca_per_month(spark: SparkSession) -> DataFrame:
    ...
    df = df.groupBy(F.month(F.to_date("Date_Vente", "dd/MM/yyyy"))).agg(F.sum("Montant_Total_EUR").alias("CA"))
    return df
...
def ca_per_month_shop(spark: SparkSession) -> DataFrame:
    ...
    df = spark.table("workspace.silver.silver_table")
    df = df.withColumn("Montant_Total_EUR", F.when(F.col("Devise") == "USD", F.col("Montant_Total") * USD_EUR).when(F.col("Devise") == "EUR", F.col("Montant_Total")).otherwise(F.col("Montant_Total") * JPY_EUR))
    df = df.groupBy('Nom_Boutique', F.month(F.to_date("Date_Vente", "dd/MM/yyyy"))).agg(F.sum("Montant_Total_EUR").alias("CA"))
    return df
...
def aggregate_data(
    ...
    df1.write.format("delta").mode("append").saveAsTable(f"{gold_schema}.ca_per_month")
    df2 = ca_per_month_shop(spark)
    df2.write.format("delta").mode("append").saveAsTable(f"{gold_schema}.ca_per_month_and_shop")
    ...
    df4.write.format("delta").mode("append").saveAsTable(f"{gold_schema}.top_selling_products_by_ca")
